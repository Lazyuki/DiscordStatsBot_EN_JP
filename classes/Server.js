...
const N3plus = /[当誰俺難違僕他付伝位例全公内初助化君和変丈夫失守昔昨最未末様次然的直石礼笑米糸絵美良虫血負部配面願]/
...
module.exports = class Server {
    ...
    addMessage(message) {
      ...
      if (!message.member) {
        console.log(`#${message.channel.name}, by ${JSON.stringify(message.author)}, ${message.content}`);
        return;
      }
    ...
    checkBegJp(message) {
      ...
      let content = message.content.replace(Util.REGEX_URL, '');
