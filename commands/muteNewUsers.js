...
module.exports.command = async (message, content, bot, server) => {
  if (!message.member.hasPermission('ADMINISTRATOR')) return;
	...
	if (nu.hasPermission('SEND_MESSAGES')) { // Get rid of all permissions.
		nu.setPermissions(['READ_MESSAGE_HISTORY']);
		server.guild.defaultRole.setPermissions(['READ_MESSAGE_HISTORY']);
		...
		nu.setPermissions(['ADD_REACTIONS', 'SEND_MESSAGES', 'EMBED_LINKS', 'ATTACH_FILES',
		'READ_MESSAGE_HISTORY', 'USE_EXTERNAL_EMOJIS', 'CHANGE_NICKNAME']);
		server.guild.defaultRole.setPermissions(['ADD_REACTIONS', 'SEND_MESSAGES', 'EMBED_LINKS', 'ATTACH_FILES',
		'READ_MESSAGE_HISTORY', 'USE_EXTERNAL_EMOJIS', 'CHANGE_NICKNAME']);
