...
module.exports.alias = ['id', 'sf', 'snowflake'];
...
module.exports.help =
  'Shows the Discord ID for an object and the creation time. `,id <@mention, #channel, ID, or username>`';

module.exports.command = async (message, content, bot, server) => {
  if (content == '') {
    message.channel.send(
      "Give me either `@mention`, `#channel`, `valid snowflake ID`, or the user's name"
    );
    return;
  }
  //TODO: roles
  ...
  if (userMention) {
    for (var u of users.keys()) {
      ...
      let dateStr = dateFormat(date, "UTC:ddd mmm dS, yyyy 'at' h:MM TT");
      embed.title = 'Creation time in UTC and your local time';
      embed.setAuthor(users.cache.get(u).tag, users.cache.get(u).avatarURL());
      embed.description =
        'Snowflake ID: ' +
        u +
        ` (<@${u}>)\nUnix Time in milliseconds: ${date.getTime()}`;
