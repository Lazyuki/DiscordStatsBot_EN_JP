...
module.exports.alias = ['user', 'u'];
module.exports.isAllowed = () => {
  return true;
};

module.exports.help =
  '`,u [name, @mention]` Defaults to the invoked user. Note that the name search only works if the user has said something in the past 30 days. Else, @mention them.';
...
module.exports.command = async (message, content, bot, server) => {
  ...
  let count = 0;
  let week = 0;
  for (let i = server.today; i >= server.today - 28; i--) {
    // 4 weeks
  ...
  let jpnPercent = jp
    ? ((record.en / (record.jp + record.en)) * 100).toFixed(2)
    : ((record.jp / (record.jp + record.en)) * 100).toFixed(2);
  embed.addField('Messages sent M | W', `${record.thirty} | ${week}`, true);
  if (server.guild.id != '206599473282023424' && !isNaN(jpnPercent))
    embed.addField(
      jp ? 'English usage' : 'Japanese usage',
      jpnPercent + '%',
      true
    ); // ignore Eikyuu server
