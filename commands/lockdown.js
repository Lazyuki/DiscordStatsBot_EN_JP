...
module.exports.command = async (message, content, bot, server) => {
  ...
  let lockdown =
    !timestamp_id && !link && !regex ? null : { link, regex, ignoreCase };
  if (timestamp_id) {
    lockdown.after = Discord.SnowflakeUtil.deconstruct(
      timestamp_id
    ).date.getTime();
  }
  ...
  try {
    ...
    message.channel.send(
      'Failed to overwrite permissions for MEE6 and Rai in JHO'
    );
    return;
  }

  message.channel.send(
    `✅ Server is now under lockdown. Mee6 and Rai have been muted in JHO. (Time ID: ${
      lockdown.after
    }, Link: ${lockdown.link}, RegEx: /${lockdown.regex}/${
      ignoreCase ? 'i' : ''
    })`
  );
};
